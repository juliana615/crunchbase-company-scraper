...
class Crunchbase:
    ...
    def login(self, email=None, password=None):
        ...
        if email is not None and password is not None:
            ...
            email_input = self.__driver.find_elements(By.XPATH, "//input[@name='email']")
            email_input[0].send_keys(email)

            password_input = self.__driver.find_elements(By.XPATH, "//input[@name='password']")
            password_input[0].send_keys(password)

            login_button = self.__driver.find_elements(By.XPATH, "//button[@type='submit']")
            login_button.click()
            time.sleep(10)
